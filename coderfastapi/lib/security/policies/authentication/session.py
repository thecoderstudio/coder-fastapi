...
class UserSessionAuthenticationPolicy(AuthenticationPolicy):
    """Authentication policy that identifies users via session cookies.

    Sessions are stored in Redis.
    """

    @classmethod
    async def authenticate_request(  # ty: ignore[invalid-method-override]
        cls,
        request: T,
    ...
    def _authenticate_from_user_session(request: T, session: UserSession) -> T:
        request_ = copy.copy(request)
        request_.user_id = session.user_id  # ty: ignore[unresolved-attribute]
        return request_
