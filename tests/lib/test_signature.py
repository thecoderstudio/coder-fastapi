...
from coderfastapi.lib.signature import check_parameter_has_default, copy_parameters
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            signature(method_with_default_params),
            ["d", "e", "f"],
            ["e", "d", "a", "b", "f", "c"],
        ),
...
@pytest.mark.parametrize(
    "parameter, expected_outcome",
    [
        (Parameter("test", Parameter.POSITIONAL_OR_KEYWORD, default="test"), True),
        (
            Parameter("test", Parameter.POSITIONAL_OR_KEYWORD, default=Parameter.empty),
            False,
        ),
    ],
)
